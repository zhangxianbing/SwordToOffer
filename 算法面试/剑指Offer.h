 ...
 * @Date: 2019-08-09 11:42:18
 * @LastEditors: zhangxianbing
 * @LastEditTime: 2019-08-09 11:42:18
...
vector<string> Permutation(string str) {
  ...
  do {
    res.push_back(str);
    int j = n - 1;
    while (j - 1 >= 0 && str[j - 1] >= str[j]) j--;  // 注意>=
    if (j == 0) break;
    int i = j;
    while (str[i] > str[j - 1]) i++;
...
long util(vector<int>& raw, vector<int>& data, int l, int r) {
  ...
  long res = 0, res_l = util(raw, data, l, m),
       res_r = util(raw, data, m + 1, r);
  for (int i = l; i <= r; i++) raw[i] = data[i];  // 注意复制在调用util之后
  while (k <= r) {
