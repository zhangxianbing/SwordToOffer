 ...
 * @LastEditTime: 2019-08-09 11:42:51
...
void moveZeroes(vector<int>& nums) {
  ...
  for (int i = 0; i < nums.size(); i++) {
    if (nums[i] != 0) {
      if (j < i) nums[j] = nums[i];  // 优化1：减少不必要的赋值
      j++;
    }
  }
  for (; j < nums.size(); j++) {
...
bool isPalindrome(string s) {
  ...
  while (l < r) {
    ...
    if (l == n) break;
    while (!isalnum(s[r])) r--;
    if ((s[l++] & 0xDF) != (s[r--] & 0xDF)) return false;
  }
  return true;
}
}  // namespace LC125
...
int findKthLargest(vector<int>& nums, int l, int r, int K) {
  ...
  if (K <= i) return findKthLargest(nums, l, i, K);
  if (K >= j) return findKthLargest(nums, j, r, K);
  return nums[i + 1];
}
